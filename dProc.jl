  ...
  for i in 1:gn
    ...
    for j in keys(genCost[(genLoc,genName)])
      if j != 9
        ...
        genα = genCost[(genLoc,genName)][j];
      end
    end

    genItem = genData(genID,genName,genLoc,genCn,gencParams,genPmax,genPmin,genQmax,genQmin,genα);
...
function branchProc(baseMVA,branchSeg,transformerSeg)
  ...
  for i in 1:brn
    ...
    brτprime = 1;
    brθtr = 0;
    brItem = branchData(brFrom,brTo,brCKT,branchID,bRevID,brr,brx,brg,brb,brbc,brt,brZeroImpe,brτ,brτprime,brθtr);
    brRevItem = branchData(brTo,brFrom,brCKT,bRevID,branchID,brr,brx,brg,brb,brbc,brt,brZeroImpe,brτ,brτprime,brθtr);
